...
import {
  ...
  TOPDOWN_VIEW_H,
  TOPDOWN_VIEW_W,
  type RoomFootprintKind,
} from "@/data/isometricFloor";
import { departmentFill, FLOOR_ROW_BOTTOM, FLOOR_ROW_TOP } from "@/lib/floorLayout";
import { openTaskCountFor, type DeptTask } from "@/stores/departmentTaskStore";
...
function RoomCell({
  ...
  slug: string;
  i: number;
  onRoom: (s: string) => void;
  openTasks: number;
  label: string;
}) {
  const plan = ISO_ROOM_PLAN[slug];
  if (!plan) return null;
  const r = planRectToTopDownSvg(plan);
  const cx = r.x + r.width / 2;
  const cy = r.y + r.height / 2;
  const footprint = (ROOM_FOOTPRINT[slug] ?? "chamfer") as RoomFootprintKind;
  const outline = roomFootprintPath(footprint, r.x, r.y, r.width, r.height);
  const fill = departmentFill(slug);
  ...
  return (
    ...
    >
      <path d={outline} fill={fill} stroke="#0f172a" strokeWidth={2} strokeLinejoin="round" />
      {(() => {
      ...
      <path
        ...
        role="button"
        tabIndex={0}
        aria-label={`Open ${label}${openTasks ? `, ${openTasks} open tasks` : ""}`}
        onClick={() => onRoom(slug)}
        onKeyDown={(e) => {
...
export function TopDownFloorPlan({
  ...
  tasksByDept: Partial<Record<string, DeptTask[]>>;
  ...
  const labelFor = (slug: string) => deptLabels[slug] ?? slug;
  ...
  return (
    <div className="iso-floor-stage w-full overflow-hidden">
      ...
      >
        ...
        {FLOOR_ROW_TOP.map((slug, i) => (
          <RoomCell
            key={slug}
            slug={slug}
            i={i}
            onRoom={onRoom}
            openTasks={openTaskCountFor(slug, tasksByDept)}
            label={labelFor(slug)}
          />
        ))}
