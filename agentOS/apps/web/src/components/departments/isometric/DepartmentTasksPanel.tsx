...
export function DepartmentTasksPanel({ slug }: { slug: string }) {
  const [draft, setDraft] = useState("");
  const tasks = useDepartmentTaskStore((s) => s.byDept[slug] ?? []);
    ...
    <div className="iso-card overflow-hidden">
      ...
      <div className="space-y-3 p-3">
        <p className="text-[11px] leading-relaxed text-slate-500">
          Tasks stay in this browser (saved locally). Open tasks appear as badges on the floor map for on-floor
          departments.
        </p>
            ...
            {tasks.map((t) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={t.done}
                  onChange={() => toggleTaskDone(slug, t.id)}
