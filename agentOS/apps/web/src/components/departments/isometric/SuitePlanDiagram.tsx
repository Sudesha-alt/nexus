...
import { roomFootprintPath } from "@/data/isometricFloor";
import { departmentFill } from "@/lib/floorLayout";

export function SuitePlanDiagram({ slug, deptName }: { slug: string; deptName: string }) {
  const id = useId().replace(/:/g, "");
  const hatchId = `suite-hatch-${id}`;
  const zoneA = departmentFill(slug, 0.85);

  const outer = roomFootprintPath("chamfer", 16, 16, 408, 268);
  const spine = roomFootprintPath("trapezoid", 18, 18, 404, 42);
  const zoneAPath = "M 20 60 L 290 60 L 290 186 L 234 186 L 234 278 L 20 278 Z";
  ...
  return (
    <div className="iso-card overflow-hidden">
      ...
      >
        ...
        >
          SUITE PLAN · {deptName.toUpperCase()}
        </text>
      </svg>
    </div>
  );
